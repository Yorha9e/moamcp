...
6. **Archive.** Once every debater has completed, call `mcp__moamcp__moa_complete(task_id)` — it writes the four-layer archive (`probe.json`, `events.jsonl`, `result.json`, `board.jsonl`) to `logs/{task_id}/`.
7. **Report.** Your final report must include: the `task_id` and the live card URL (`http://127.0.0.1:<port>/?task_id=<task_id>` — query the Bus port/mode with `mcp__moamcp__moa_status`, which returns it in the `bus` field; the workspace `bus.port` file is only kept for legacy compatibility) so a latecomer can replay the debate; transcript excerpts (the key points of each round); the archive path (`logs/{task_id}/`); and the VERDICT block (format unchanged).
