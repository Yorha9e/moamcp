...
- **The stdio bridge is the standard fallback for tower calls.** Subagent sessions may lack the `mcp__moamcp__moa_tower_*` tool surface entirely. Ship a bridge script (e.g. `tmp/tower-cli.mjs` speaking MCP over stdio to the plugin's `dist/server.js`, payload passed as `@file`) into the worktree, and have workers/reviewers place their phase-2 tower calls through `Bash` with their real `caller_agent_id`. Use the bridge yourself when your own surface lacks the tools. Never fabricate a submission on an agent's behalf without the agent executing the call.
